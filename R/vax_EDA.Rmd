...
wd <- "/Users/danielpringle/Code/covidvaxnz/R"
setwd(wd)
```
...
p_v_nz_week <- ggplot(data = v_nz, aes(x = week, y = count, color = dose)) +
  ...
  labs(y = "",
       x = "Week ended") +
  ggtitle("Weekly number of first and second vaccinations received") +
  theme_minimal() +
...
p_v_nz_total <- ggplot(data = v_nz, aes(x = week, y = total, color = dose)) +
  ...
  labs(y = "",
       x = "") +
  ggtitle("Total number of first and second vaccinations received to date") +
  theme_minimal() +
  theme(legend.position=c(0.2,0.5), 
        legend.direction = "horizontal")
...
p_v_nz_uptake <- ggplot(data = v_nz, aes(x = week, y = uptake, color = dose)) +
  ...
  labs(y = "",
       x = "") +
  ggtitle("Uptake of first and second vaccinations") +
  theme_minimal() +
  theme(legend.position=c(0.2,0.5), 
        legend.direction = "horizontal")
...
v_dhb <- vx[ , .(count = sum(count),
                pop = sum(popsegment)), .(week, dose, DHB)]
setorderv(v_dhb, c("DHB", "week", "dose"))
v_dhb[ , `:=`(total = cumsum(count)), .(dose, DHB)]
v_dhb[ , `:=`(uptake = round(total/pop, 3)), .(dose, DHB)]
...
p_v_dhb_uptake <- ggplot(data = v_dhb, aes(x = week, y = uptake, color = dose)) +
  ...
  ggtitle(paste0("Uptake of first and second vaccinations, at ", format(max(vx$week), "%d %B %Y"))) +
  facet_wrap(~DHB) + 
  theme_bw() +
